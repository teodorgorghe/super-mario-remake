...
struct PipeComponent : public Component {
    explicit PipeComponent(Teleport teleport, int teleportX, int teleportY, WorldID world, Theme::Id theme) {
...
struct PipeTravelComponent : public Component {
};
...
struct FlowerMarioComponent : public Component {
};

struct FlowerComponent : public Component {
};
...
struct QuestionBlockComponent : public Component {
    explicit QuestionBlockComponent(
        ...
        bool oneup = false,
        bool flower = false
    ) : spawn{spawn}, coin{coin}, oneup{oneup}, flower(flower) {}
...
struct TransferStatsComponent : public Component {
    size_t index; // Scene index
    std::bitset<3> transferBitset; //First bit - score transfer, second bit - time transfer, third bit - lives transfer
};
