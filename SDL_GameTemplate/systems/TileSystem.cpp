...
//Spawn flower
void createFlower(World* world, Entity* block) {
    auto player = world->findFirst<PlayerComponent>();

    if (!player->has<SuperMarioComponent>() && !player->has<FlowerMarioComponent>())
    {
        createMushroom(world, block);
        return;
    }

    auto mushroom = world->create();
    mushroom->assign<TextureComponent>(TextureId::FLOWER);
    ...
    mushroom->assign<AnimationComponent>(
    ...
    mushroom->assign<FlowerComponent>();
    mushroom->assign<GrowComponent>();
}

// Create coin animation from the question block
...
void TileSystem::tick(World* world) {
    ...
    for (auto entity : world->find<BreakableComponent>()) {
        ...
        if (!breakable->finished()) {
            ...
            if (entity->has<QuestionBlockComponent>()) {
                if (entity->get<QuestionBlockComponent>()->flower) createFlower(world, entity);
