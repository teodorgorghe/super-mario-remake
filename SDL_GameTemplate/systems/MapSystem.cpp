...
void MapSystem::tick(World* world) {
    ...
    // Stergere entitati in afara camerei
    ...
    // Instantiere entitati dupa ce au fost vizionate in cadru
    // Preluare date din harta (coada) si crearea obiectelor pe categorii.
    while (!map.tiles.empty()) {
        ...
        else if (tile->hasProperty(COIN))
        {
            entity->assign<CoinComponent>();
            entity->assign<AnimationComponent>(
                std::vector<TextureId>{
                    TextureId::COIN_ITEM_1,
                    TextureId::COIN_ITEM_2,
                    TextureId::COIN_ITEM_3,
                },
            12);
        ...
        if (tile->hasProperty(KINETIC)) {
            ...
            {
                auto pipe = map.pipes.front();
                entity->assign<PipeComponent>(pipe->teleport, pipe->posX, pipe->posY, pipe->worldID, pipe->theme);
                delete pipe;
                map.pipes.pop();
            } else {
